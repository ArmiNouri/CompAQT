...
class DataLoaderSimple:
    ...
    def get_batch(self):
        ...
        for each_data in self.data[start_index: end_index]:
            ...
            # batch_data["option_mask"].append(each_data.option_mask)
            batch_data["question_mask"].append(each_data.question_mask)
            batch_data["facts_mask"].append(each_data.facts_mask)
            batch_data["numbers_mask"].append(each_data.numbers_mask)

            batch_data["unique_id"].append(each_data.unique_id)
            batch_data["example_index"].append(each_data.example_index)

            batch_data["question_tokens"].append(each_data.question_tokens)
            batch_data["question_ids"].append(each_data.question_ids)
            batch_data["facts_tokens"].append(each_data.facts_tokens)
            batch_data["facts_ids"].append(each_data.facts_ids)
            # batch_data["segment_ids"].append(each_data.segment_ids)
            # batch_data["options"].append(each_data.options)
            batch_data["answer"].append(each_data.answer)
            batch_data["program"].append(each_data.program)
            ...
            batch_data["program_weight"].append(each_data.program_weight)
            batch_data["program_mask"].append(each_data.program_mask)
            batch_data["operator_ids"].append(each_data.operator_ids)
            batch_data["operator_mask"].append(each_data.operator_mask)
...
def evaluate_result(json_in, json_ori, all_res_file, error_file, program_mode):
    ...
    for tmp in data:
        ...
        if equal_program(gold, pred):
            ...
            assert exe_res == gold_res or abs(exe_res - gold_res) < 0.01
            prog_correct += 1
